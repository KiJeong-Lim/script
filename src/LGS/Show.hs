...
genLexer xblocks = do
    ...
    fmap (strcat . snd) $ runWriterT $ do
        ...
        if Map.size (getTransitionsOfDFA theDFA) < 10
            then do
                sequence
        ...
        sequence
            [ case destructor of
                Just (hscode : hscodes)
                    | null hscodes -> do
                        tellLine (strstr "            ((" . showsPrec 0 line . strstr ", this), ((row1, col1), (row2, col2))) -> returnJust (" . strstr hscode . strstr ")")
                        return ()
                    | otherwise -> do
                        tellLine (strstr "            ((" . showsPrec 0 line . strstr ", this), ((row1, col1), (row2, col2))) -> returnJust $ " . strstr hscode)
