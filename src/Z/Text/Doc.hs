...
renderDoc :: Bool -> Doc -> String
renderDoc beauty
    | beauty = render . toViewer . reduce
    | otherwise = render . toViewer . reduce . remove
    where
        remove :: Doc -> Doc
        remove DE = DE
        remove (DT str1) = DT str1
        remove (DS style1 doc1) = remove doc1
        remove (DC color1 doc1) = remove doc1
        remove DB = DB
        remove (DV doc1 doc2) = DV (remove doc1) (remove doc2)
        remove (DH doc1 doc2) = DH (remove doc1) (remove doc2)
        reduce :: Doc -> Doc
        reduce DE = DE
